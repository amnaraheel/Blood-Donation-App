...
class grid_layout_used(GridLayout):
 ...
 self.clear_widgets()
 self.cols = 1
 self.donor_button = Button(text="DONOR INFO", 
background_color =(0,1,1,1),color =(1, 1, 1, 1))
 self.donor_button.bind(on_press=self.on_click_donor)
 self.reciver_button = Button(text="RECIEVER INFO" , 
background_color =(0,1,1,1),color =(1, 1, 1, 1))
...
self.add_widget(Label(text=F'{arr[0]}{arr[1]}{arr[2]}{arr[3]}'))
 ...
 self.clear_widgets()
 self.cols=1
 self.add_widget(Label(text="Have you Recieved the 
Blood?",font_size="15sp"))
...
arr=active_list.delete_send_informtion(int(self.index.text))
 ...
 self.button1 = Button(text="Submit", background_color=(0, 
1, 1, 1),color=(1, 1, 1, 1))
 self.button1.bind(on_press=self.on_submit)
 self.button2 = Button(text="Exit", background_color=(0, 1, 
1, 1), color=(1, 1, 1, 1))
 ...
 self.text3 = Label(text="City (where is he currently 
available)")
 ...
 FILE.close()
 #creating data to be stored in active list Adding a new 
line after eachh variable
 self.name.text=self.name.text+"\n"
 self.phone.text=self.phone.text+"\n"
 self.blood.text=self.blood.text+"\n"
 self.city.text=self.city.text+"\n"
 active_list.create_node(self.name.text, 
self.phone.text,self.blood.text,self.city.text)
 self.start_tab()
 return
...
class Node:
 ...
 def __init__(self, name=None, phone_num=None, blood_type=None, 
city=None, next=None, pre=None):
 self.name = name
...
class data_collection_Dlinkedlist:
 ...
 self.counter=0
 return
 # creating a node for storing data of the donor (we are 
creating node at the start)
 ...
 temp = temp.next
 return
 #this function is use to take information from the node and 
return it in list form
 ...
 return
 #returing the information store in the node that is to be 
deleted so that we can store
 #it in inactive file
 ...
 li.append(temp.city)
 return li
# CREATING GLOBAL OBJECT OF DATA COLLLECTION CLASS SO THAT WE CAN 
USE IT IN ANYOTHER CLASS
global active_list
...
if __name__ == "__main__":
 ...
 city = i
 active_list.create_node(name, contact_num, blood_type, 
city)
 y = 0
 FILE.close()
 #closing file
